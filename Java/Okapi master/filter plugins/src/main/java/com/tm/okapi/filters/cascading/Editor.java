...
public class Editor implements IParametersEditor {
	...
	public String createXliff(String sFilePath, IFilter filter, String sOutXliffPath) throws Exception {
		...
		try {
			...
			segParams.setSourceSrxStream(inputStream);
			segParams.setTargetSrxStream(inputStream);
			...
			driver.addBatchItem(rawDoc, new java.io.File(sOutXliffPath).toURI(), "UTF-8");
			// Process
			driver.processBatch();

			String sXliffContent = Files.readString(Path.of(sOutXliffPath));
			long lEnd = System.currentTimeMillis() - lStart;
			//System.out.println(DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss").format(LocalDateTime.now()) +
			//		" createXliff: " + sFilePath + " -> " + lEnd + "ms");

			return sOutXliffPath;
		} catch (Exception ex) {
